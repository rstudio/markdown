...
output:
  markdown::html_format:
    meta:
      css: [default, snap, "@npm/@xiee/utils/css/key-buttons.css"]
      js: [snap, "@npm/@xiee/utils/js/key-buttons.js,external-link.js"]
    options:
      toc: true
      highlight_code:
        style: xcode
...
You need at least one CSS file (`snap.css`) and one JS script (`snap.js`), which
can be specified in YAML metadata:
...
``` r
markdown::mark_html("test.md", meta = list(
  css = c("default", "snap"),
  js = "snap"
))
```
...
2.  Or use the second-level section header (`##`).

You must choose only one way, i.e., either use horizontal rules to separate all
slides, or avoid horizontal rules but only use section headers.
...
-   `f`: Fullscreen mode (press `Esc` to exit). The scroll bar will be hidden in
    the fullscreen mode. You can also use the keyboard shortcut of your browser,
    e.g., `fn + F` in Chrome on macOS.
...
-   `m`: Mirror mode
    ...
    -   Slides are mirrored ([why this odd
        feature?](https://yihui.org/en/2020/04/xaringan-mirror/)).
...
By default, sections (`<h2>`) are numbered via a [CSS
counter](https://www.w3schools.com/css/css_counters.asp). If you do not want the
numbers, you can hide them via CSS:
...
| Desktops            | ≥ 992px  | Slide |   200%    |        2         |
| Phones and tablets  | \< 992px | Page  |   100%    |       N/A        |

You can resize your browser window to see the effect (also try to press `o` and
test the overview mode). If you are on a mobile device, you should see a normal
continuous page, since you cannot adjust the window size.
...
(`Cmd + Shift + C` in RStudio's visual Markdown editor), e.g.,
...
You can define your own arbitrary class names and corresponding CSS rules, e.g.,
`<!--# class="large" -->` and `.large { font-size: 150%; }`.
...
<!--# style="background-image: url(https://user-images.githubusercontent.com/163582/219277796-6d2dd826-fb11-4970-b07c-dd920a694e3b.jpg); background-size: cover; color: white;" -->
...
`contenteditable` attribute. Click on this page and try to edit any text.
...
The timer will start when the presentation enters the fullscreen mode.

To restart the timer, click on it.
...
The aspect ratio of slides is not defined by default. All available space on the
screen will be used to arrange the content. If you present the slides directly
with your own computer, this may not be a problem since you know if all content
fits well on your own screen. However, if you connect your computer to a
projector or present the slides on another computer, you'd better know the
screen size beforehand, because the aspect ratio may be different, and your
slides can look different on that screen.
...
<link href="https://cdn.jsdelivr.net/combine/gh/rstudio/markdown@VERSION/inst/resources/default.min.css,gh/rstudio/markdown@VERSION/inst/resources/slides.min.css" rel="stylesheet"></link>
```

Remember to substitute `VERSION` with an appropriate version (e.g., `1.6`):
<https://github.com/rstudio/markdown/tags> You can omit `@VERSION` to use the
latest version but it is not recommended because future updates might break your
old slides.

Both the JS and CSS are quite lightweight (total size is \~8Kb when
uncompressed) and have no dependencies. They were written from scratch.

--------------------------------------------------------------------------------

## Enjoy!

<!--# style="background-image: url(https://upload.wikimedia.org/wikipedia/commons/7/7e/Mudra-Naruto-KageBunshin.svg); background-position: bottom right; background-repeat: no-repeat;" -->

-   I developed the [xaringan](https://github.com/yihui/xaringan) in package
    2016 and have used it since then. It was based on the JS library
    `remark.js`, which still works perfectly, but now I prefer more lightweight
    frameworks.

-   With less than 300 lines of JS/CSS code, I have gotten most of presentation
    features that I need.

-   This simple presentation framework is quite customizable.

-   If you know HTML/CSS/JS, you can do more advanced customization.

-   Github repo:
    [https://github.com/rstudio/markdown](https://github.com/rstudio/markdown/tags)
